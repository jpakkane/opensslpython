...
output = 'pyout.asm' #$#ARGV >= 0 && $ARGV[$#ARGV] =~ m|\.\w+$| ? pop : undef;
...
def hi(r):
    return re.sub('%[er]([a-d])x', r'%\1h', r)

def lo(r):
    r = re.sub('%[er]([a-d])x', r'%\1l', r)
    r = re.sub('%[er]([sd]i)', r'%\1l', r)
    r = re.sub('%(r[0-9]+)[d]?', r'%\1b', r)
    return r
...
def deckey_ref(i, ptr, te, td):
    (tp1, tp2, tp4, tp8, acc) = ("%eax","%ebx","%edi","%edx","%r8d");
    code+='''
	mov	$i($ptr),$tp1
	mov	$tp1,$acc
	and	\$0x80808080,$acc
...
keyp="0(%rsp)"		# one to pass as $key
keyend="8(%rsp)"		# &(keyp->rd_key[4*keyp->rounds])
_rsp="16(%rsp)"		# saved %rsp
_inp="24(%rsp)"		# copy of 1st parameter, inp
_out="32(%rsp)"		# copy of 2nd parameter, out
_len="40(%rsp)"		# copy of 3rd parameter, length
_key="48(%rsp)"		# copy of 4th parameter, key
_ivp="56(%rsp)"		# copy of 5th parameter, ivp
ivec="64(%rsp)"		# ivec[16]
